...
static int is_domain_name_safe(const char *);
...
int main(int argc, char *argv[])
{
	...
	char username[256];
	snprintf(username, sizeof(username), "nobody");
...
#define _CHDR_SIZE CMSG_SPACE(sizeof(struct in6_pktinfo)) + CMSG_SPACE(sizeof(int))
	for (;;) {
		...
		if (len > 0) {
			...
			if (!pkt_info) {
				flog(LOG_WARNING, "received packet without IPV6_PKTINFO, ignoring");
				continue;
			}
...
static void print_ff(unsigned char *msg, int len, struct sockaddr_in6 *addr, int hoplimit, unsigned int if_index, int edefs)
{
	...
	if (len < (int)sizeof(struct nd_router_advert)) {
		flog(LOG_WARNING, "received RA with invalid length (%d) from %s", len, addr_str);
		return;
	}
	...
	while (len > 0) {
		...
		case ND_OPT_TIMESTAMP: {
			...
			if (optlen < 16) {
				flog(LOG_ERR, "truncated timestamp option from %s", addr_str);
				break;
			}
	...
	while (orig_len > 0) {
		...
		case ND_OPT_PREFIX_INFORMATION: {
			...
			if (optlen < (int)sizeof(*pinfo)) {
				flog(LOG_ERR, "truncated prefix information option from %s", addr_str);
				break;
			}
		...
		case ND_OPT_DNSSL_INFORMATION: {
			...
			for (int offset = 0; offset < (dnssl_info->nd_opt_dnssli_len - 1) * 8;) {
				...
				if (label_len == 0) {
					...
					/* DNSSL suffixes are printed bare (the config grammar does not
					 * accept quoted ones), so anything outside the hostname
					 * character set would be parsed as config syntax. Omit it. */
					if (is_domain_name_safe(suffix)) {
						printf(" %s", suffix);
					} else {
						flog(LOG_ERR, "DNSSL suffix with invalid characters from %s, omitted", addr_str);
						nskipped++;
					}
			...
			if (nskipped)
				printf(" # %d suffix(es) with invalid characters omitted", nskipped);
		...
		case ND_OPT_DNR_INFORMATION: {
			...
			/* quoted: the grammar strips the quotes, and print_sanitized escapes
			 * anything that could end the string early */
			printf("\n\tDNR \"");
...
/* Only letters, digits, '-' and '.' are accepted by the DNSSL config grammar. */
static int is_domain_name_safe(const char *s)
{
	for (; *s; s++) {
